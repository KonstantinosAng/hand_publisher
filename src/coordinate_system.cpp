...
int main() {
  ...
  // setup vectors to hold the chessboard corners in the chessboard coordinate
  ...
  // set up VideoCapture object to acquire the webcam feed from location 0
  // (default webcam location)
  ...
  while (!doneYet) {
    ...
    bool found =
        findChessboardCorners(gray, cbSize, imagePoints, CALIB_CB_FAST_CHECK);
    ...
    if (found) {
      ...
      solvePnP(Mat(boardPoints), Mat(imagePoints), intrinsics, distortion, rvec,
               tvec, false);
      ...
      circle(webcamImage, (Point2d)imagePoints[0], 4, CV_RGB(255, 0, 0));
      ...
      cout << fixed << setprecision(2) << "rvec = [" << rvec.at<double>(0, 0)
